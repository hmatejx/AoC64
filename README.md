...
- Some building blocks, most importantly `reucpy.h`, were kindly shared with me by Jukka. Without that REU DMA helper, many solutions would just have been impractical.
...
Not every puzzle deserves an expanded story. I have clearly marked (with a ⚠) and underlined the days that were especially challenging given the C64 constraints. Here is the summary.
...
- For hashing a structure of type `Key` named `key` you define:
  ...
  - You can use `pearson.h` to build compact and MOS6502-friendly hashes for 8, 16, and 24-bit hash sizes.
- Utilities:
  - `tick(n & 7)` just animates an 8-frame spinner which handy to observe long runs.
...
Headers: `lib/hashset.h`, `lib/hashset_reu.h` (REU)

```c++
#include "../lib/hashset.h"
#include "../lib/pearson.h" // for hash16/hash24 if you want

struct Point {
  int16_t x,y;
  uint16_t hash() const { return (x*31 + y) & 0x3FF; } // example, match capacity mask
};
...
Header: `lib/hashmap.h`,`lib/hashmap_reu.h` (REU)
...
struct Key {
  uint8_t a,b,c,d;
  uint16_t hash() const { return hash16((const uint8_t*)this, 4); }
};
...
Headers: `lib/min_heap.h`, `lib/min_heap_reu.h` (REU)
...
memset_reu(0, (uintptr_t)buf, N);       // REU → RAM: clear N bytes to 0
memcpy_reu(dst, src, N);                // REU-accelerated copy (RAM↔RAM or REU↔RAM depending on wrapper)
reu_clear(banks_needed(num_bytes));     // clear REU region (project helper)
```
